...
const seed = async (count) => {
    ...
    const csvWriter =  createCsvWriter({
      path: path.join(__dirname + "/../../db/Cassandra/reviews.csv"),
      header: [
        {id: 'id', title: 'id'},
        {id: 'bId', title: 'bId'},
        {id: 'uId', title: 'uId'},
    ...
    csvWriter.writeRecords(reviews)
        .then(() => {
          ...
          const pathtoFile = path.join(__dirname + "/../../db/Cassandra/reviews.csv");
          // COPY reviews (id, uId, bId, rating, reviewText, checkin, useful, funny, cool) FROM '/Users/josevaladez/Documents/Rpt13/SDC/NickRashad_FEC/db/Cassandra/reviews.csv' WITH DELIMITER=',' AND HEADER=TRUE;
          let command = `cqlsh 127.0.0.1 9042 -e "COPY yelp.reviews (id, bId, uId, rating, reviewText, checkin, useful, funny, cool) FROM '${pathtoFile}' WITH DELIMITER=',' AND HEADER=TRUE;"`;;
          exec(command, { encoding: 'utf8', timeout: 0, maxBuffer: 5000 * 500, killSignal: 'SIGTERM', cwd: null, env: null } ,(err, stdout, stderr) => {
            ...
            if (err) {
              ...
              fs.unlink(path.join(__dirname + "/../../db/Cassandra/reviews.csv"), (err) => {
                if (err) {
                  console.log(err);
                } else {
